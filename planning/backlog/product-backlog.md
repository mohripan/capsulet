...
- Execute jobs through Kubernetes Jobs. Planned for Sprint 003.
- Capture bounded run logs. Planned for Sprint 003.
- Store script bundles, large logs, and artifacts in object storage.
- Add CLI status and logs commands. Planned for Sprint 003.
- Add run cancellation.
- Add retry policy and timeout hardening.
...
- Add Helm-defined execution pools. Initial values added in Sprint 001; runner application planned for Sprint 003.
- Apply pool resource defaults. Planned for Sprint 003.
- Apply node selectors, tolerations, and affinity. Planned for Sprint 003.
- Add pool-level concurrency limits.

## Install and Operability

- Add local Kubernetes runner guide. Planned for Sprint 003.
- Make the chart support local runner configuration. Planned for Sprint 003.
- Add metrics for queue depth and worker outcomes.
- Add lease expiry recovery.

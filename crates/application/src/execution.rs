//! Worker lease-and-run application use case.
...
pub const INLINE_LOG_LIMIT_BYTES: usize = 64 * 1024;
    ...
    O: ObjectStore,
{
    ...
    let pool_limits = pools
        .pools
        .iter()
        .map(|(name, pool)| (name.clone(), pool.max_concurrent_jobs))
        .collect::<Vec<_>>();
    let Some(mut run) = store
        .lease_next_queued_run(
            worker_id,
            lease_seconds,
            &pool_limits,
            supports_reattachment,
        )
        .await
        .map_err(WorkerError::store)?
    else {
    ...
    let input_artifacts = load_upstream_artifacts(store, object_store, run.id()).await?;
...
async fn load_upstream_artifacts<S, O>(
    store: &S,
    object_store: &O,
    run_id: &capsulet_core::JobRunId,
) -> Result<Vec<InputArtifact>, WorkerError>
where
    S: WorkerStore,
    O: ObjectStore,
{
    let metadata = store
        .list_upstream_artifacts(run_id)
        .await
        .map_err(WorkerError::store)?;
    let mut inputs = Vec::with_capacity(metadata.len());
    let mut names = HashSet::with_capacity(metadata.len());
    for (producer_step_id, artifact) in metadata {
        if !names.insert(artifact.name().to_string()) {
            return Err(WorkerError::DuplicateInputArtifact(
        ...
        let bytes = object_store
            ...
            .ok_or_else(|| WorkerError::MissingObject(artifact.object_key().to_string()))?;
...
pub enum WorkerError {
    ...
    #[error("missing upstream artifact object: {0}")]
    MissingObject(String),
    #[error("multiple prerequisites produced an artifact named {0}")]

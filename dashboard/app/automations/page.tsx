...
export default function AutomationsPage() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setMessage(`Created ${automation.name}`);
  ...
  return (
    <DashboardShell>
      ...
      <div className="pageToolbar">
        <button className="primaryAction" onClick={() => setIsModalOpen(true)}>
      ...
      {isModalOpen ? (
        <div className="modalBackdrop" role="dialog" aria-modal="true" aria-label="Create automation">
          <form className="wizardModal" onSubmit={submit}>
            <div className="wizardHeader">
              <div><span>New automation</span><h2>{wizardStep === "details" ? "Details" : wizardStep === "triggers" ? "Triggers" : "Condition"}</h2></div>
              <button className="iconButton" type="button" onClick={() => setIsModalOpen(false)}>x</button>
            </div>
            ...
            <div className="wizardFooter">
              ...
              {wizardStep !== "condition" ? (
                ...
                <button className="primaryAction" disabled={isSubmitting || !jobDefinitionId || !hostGroup}><Send size={16} />{isSubmitting ? "Creating" : "Create automation"}</button>
              )}
            </div>
          </form>
        </div>

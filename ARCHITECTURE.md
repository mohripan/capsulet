...
As of M2 it is defined, admitted, certified, and replayable. As of M3 it runs, through the durable
...
- **Trust as a type.** A value's assurance is a `TrustClass`, reachable only from a verification
  ...
  combination. A record is admitted against the certificate it names, and the verdict and residual
  count are read from that certificate; whether the value crossed an unmodelled boundary is still
  the caller's word — see `CAP-IR-006`.
...
[docs/contracts/durable-run-execution.md](docs/contracts/durable-run-execution.md), and the
decisions in [ADR 0018](docs/adr/0018-durable-graph-runtime.md).
